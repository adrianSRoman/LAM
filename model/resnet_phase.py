...
class ResNet18Latent(nn.Module):
    ...
    def forward(self, x):
        ...
        # phase
        out_ph = F.relu(self.bn1_re(self.conv1_re(x_ph)))
        out_ph = self.layer1_re(out_ph)
        out_ph = self.layer2_re(out_ph)
        out_ph = self.layer3_re(out_ph)
        out_ph = self.layer4_re(out_ph)
        out_ph = F.max_pool2d(out_ph, 4)
        out_ph = out_ph.view(out_ph.size(0), -1)
        out_ph = self.linear_re(out_ph)

        latent_x = F.relu(self.linear_out3(out_ph))
