'''
Implementation adapted from the original DBPN repo: https://github.com/alterzero/DBPN-Pytorch

The present code implements a complex DBPN network (CDPN) for microphone array correlation matrix upsampling.
'''
...
class Net(nn.Module):
    ...
    def forward(self, x_rel, x_imag):
        ...
        batch_size = x_rel.size(0)
        #real
        x_rel = x_rel.view(batch_size, -1)
        x_rel = self.dense_real_1(x_rel)
        x_rel = self.dense_real_2(x_rel)
        #imag
...
#model = Net(num_channels=1, base_filter=32,  feat = 128, num_stages=10, scale_factor=2).to(device).float()
