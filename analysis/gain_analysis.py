...
gains_list = [-12, -6, 1, 6, 12] # gains to apply for analysis
...
def gain_simulation(freq_indx, use_noise=False):
    conv_signals = []
    var_dist_list = [] # variance distance list
    max_dist_list = [] # maximum pixel per distance in list
    for dB_gain in gains_list:
        gain = convert_decibels_to_amplitude_ratio(dB_gain)
        rir_tokens = list(rirs_list.keys())[0]
        clips_var_list = []
        ...
        for i, clip in enumerate(audio_clips):
            ...
            max_indices = np.argpartition(apgd_map, -12)[-12:]
            ...
            std_dist = apgd_map.std() #np.std(dists)
...
for freq_index in [0, 1, 2, 3, 4, 5, 6, 7, 8]:
    conv_signals, var_dist_list, max_dist_list = gain_simulation(freq_index, False)

    gains_labels = [-12, -6, 1, 6, 12] #[2, 2.8284271247461903, 3, 4.242640687119286]
    ...
    ax.set_title(f'Mean variance across 10 music tracks w.r.t gain \n freq band {freq_index}')
    ...
    fig.savefig(f"./figures/gain_simulation_freq{freq_index}_music.png")
