...
def main(argv):
    ...
    if params['mode'] == 'dev':
        if '2020' in params['dataset_dir']:
        ...
        elif '20.5' in params['dataset_dir']:
            test_splits = [[4]]
            val_splits = [[4]]
            train_splits = [[1, 2, 3]] 
            ...
            print("entering here??")
            test_splits = [[5]]
            val_splits = [[4]] # 0 and 6 are LOCATA and RSOANU, 6, 0]]#[[4]]
            train_splits = [[1, 2, 3]] #[[1, 2]]
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            # Print stats
            print(
                'epoch: {}, time: {:0.3f}/{:0.3f}, '
                # 'train_loss: {:0.5f}, val_loss: {:0.5f}, '
                'train_loss: {:0.4f}, val_loss: {:0.4f}, '
                'ER/F/LE/LR/SELD: {}, '
                'best_val_epoch: {} {}'.format(
                    epoch_cnt, train_time, val_time,
                    train_loss, val_loss,
                    '{:0.3f}/{:0.3f}/{:0.3f}/{:0.3f}/{:0.3f}'.format(val_ER, val_F, val_LE, val_LR, val_seld_scr),
                    best_val_epoch, '({:0.3f}/{:0.3f}/{:0.3f}/{:0.3f}/{:0.3f})'.format(best_ER, best_F, best_LE, best_LR, best_seld_scr))
            )
        ...
        print('\nTest Loss')
        print('SELD score (early stopping metric): {:0.3f} {}'.format(test_seld_scr[0] if use_jackknife else test_seld_scr, '[{:0.3f}, {:0.3f}]'.format(test_seld_scr[1][0], test_seld_scr[1][1]) if use_jackknife else ''))
        print('SED metrics: Error rate: {:0.3f} {}, F-score: {:0.1f} {}'.format(test_ER[0]  if use_jackknife else test_ER, '[{:0.3f}, {:0.3f}]'.format(test_ER[1][0], test_ER[1][1]) if use_jackknife else '', 100* test_F[0]  if use_jackknife else 100* test_F, '[{:0.3f}, {:0.3f}]'.format(100* test_F[1][0], 100* test_F[1][1]) if use_jackknife else ''))
        print('DOA metrics: Localization error: {:0.1f} {}, Localization Recall: {:0.1f} {}'.format(test_LE[0] if use_jackknife else test_LE, '[{:0.3f} , {:0.3f}]'.format(test_LE[1][0], test_LE[1][1]) if use_jackknife else '', 100*test_LR[0]  if use_jackknife else 100*test_LR,'[{:0.3f}, {:0.3f}]'.format(100*test_LR[1][0], 100*test_LR[1][1]) if use_jackknife else ''))
        if params['average']=='macro':
            ...
            for cls_cnt in range(params['unique_classes']):
                print('{}\t{:0.3f} {}\t{:0.3f} {}\t{:0.3f} {}\t{:0.3f} {}\t{:0.3f} {}'.format(
                     cls_cnt,
                     classwise_test_scr[0][0][cls_cnt] if use_jackknife else classwise_test_scr[0][cls_cnt], '[{:0.3f}, {:0.3f}]'.format(classwise_test_scr[1][0][cls_cnt][0], classwise_test_scr[1][0][cls_cnt][1]) if use_jackknife else '',
                     classwise_test_scr[0][1][cls_cnt] if use_jackknife else classwise_test_scr[1][cls_cnt], '[{:0.3f}, {:0.3f}]'.format(classwise_test_scr[1][1][cls_cnt][0], classwise_test_scr[1][1][cls_cnt][1]) if use_jackknife else '',
                     classwise_test_scr[0][2][cls_cnt] if use_jackknife else classwise_test_scr[2][cls_cnt], '[{:0.3f}, {:0.3f}]'.format(classwise_test_scr[1][2][cls_cnt][0], classwise_test_scr[1][2][cls_cnt][1]) if use_jackknife else '',
                     classwise_test_scr[0][3][cls_cnt] if use_jackknife else classwise_test_scr[3][cls_cnt], '[{:0.3f}, {:0.3f}]'.format(classwise_test_scr[1][3][cls_cnt][0], classwise_test_scr[1][3][cls_cnt][1]) if use_jackknife else '',
                     classwise_test_scr[0][4][cls_cnt] if use_jackknife else classwise_test_scr[4][cls_cnt], '[{:0.3f}, {:0.3f}]'.format(classwise_test_scr[1][4][cls_cnt][0], classwise_test_scr[1][4][cls_cnt][1]) if use_jackknife else ''))
