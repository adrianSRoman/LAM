...
def extract_subarray(arr, target_shape):
    """Extract a subarray of the given target shape from the top-left corner."""
...
class DataGenerator(object):
    def __init__(
        ...
        self._lam_feat_dir = self._feat_cls.get_lam_feat_dir()
    ...
    def _get_filenames_list_and_feat_label_sizes(self):
        ...
        print("Information on batch len", self._nb_total_batches, total_frames, self._batch_size, self._feature_seq_len)
        ...
        self._LAM_batch_seq_len = self._batch_size*self._feature_seq_len
        return
    ...
    def generate(self):
        ...
        self._circ_buf_feat_LAM = deque()
        self._LAM_bands = 0
        self._LAM_N_px = 0
        file_cnt = 0
        print("total of batches", self._nb_total_batches)
        if self._is_eval:
            print("evaluating")
            for i in range(self._nb_total_batches):
                ...
                while len(self._circ_buf_feat_LAM) < self._label_batch_seq_len:
                    temp_feat = np.load(os.path.join(self._feat_dir, self._filenames_list[file_cnt]))
                    # Last model used for table2 in latest interspeech paper revision
                    temp_feat_LAM = np.load(os.path.join(self._lam_feat_dir, self._filenames_list[file_cnt]), mmap_mode='r').transpose(1, 0, 2)

                    n_feats = temp_feat_LAM.shape[0]
                    self._LAM_bands, self._LAM_N_px = temp_feat_LAM.shape[1], temp_feat_LAM.shape[2]
                    temp_feat_LAM = temp_feat_LAM.reshape(n_feats, -1)

                    for row_cnt, row in enumerate(temp_feat):
                        self._circ_buf_feat.append(row)

                    for row_cnt, row in enumerate(temp_feat_LAM):
                        self._circ_buf_feat_LAM.append(row)
                    ...
                    if self._per_file:
                        ...
                        extra_frames_LAM = self._LAM_batch_seq_len - temp_feat_LAM.shape[0]
                        extra_feat_LAM = np.ones((extra_frames_LAM, temp_feat_LAM.shape[1])) * 1e-6

                        for row_cnt, row in enumerate(extra_feat_LAM):
                            self._circ_buf_feat_LAM.append(row)

                    file_cnt = file_cnt + 1
                    del temp_feat, temp_feat_LAM

                # Read one batch size from the circular buffer
                feat = np.zeros((self._feature_batch_seq_len, self._nb_mel_bins * self._nb_temp_featch))
                try:
                    ...
                    pass
                # Read one batch size from the LAM tokens
                feat_LAM = np.zeros((self._LAM_batch_seq_len, self._LAM_bands * self._LAM_N_px))
                for j in range(self._LAM_batch_seq_len):
                    feat_LAM[j, :] = self._circ_buf_feat_LAM.popleft()
                feat_LAM = np.reshape(feat_LAM, (self._LAM_batch_seq_len, self._LAM_bands, -1))

                # Normalize LAM maps
                feat_LAM /= feat_LAM.max()
                ...
                feat_LAM = self._split_in_seqs(feat_LAM, self._label_seq_len)
                #feat_LAM = np.transpose(feat_LAM, (0, 2, 1, 3))
                #self._circ_buf_feat = deque()
                #self._circ_buf_label = deque()
                #self._circ_buf_feat_LAM = deque()
                yield feat, feat_LAM
            ...
            print("training", self._per_file, self._feature_batch_seq_len, len(self._filenames_list))
            for i in range(self._nb_total_batches):

                # load feat and label to circular buffer. Always maintain atleast one batch worth feat and label in the
                # circular buffer. If not keep refilling it.
                while len(self._circ_buf_feat_LAM) < self._feature_batch_seq_len:
                    temp_feat = np.load(os.path.join(self._feat_dir, self._filenames_list[file_cnt]))
                    temp_label = np.load(os.path.join(self._label_dir, self._filenames_list[file_cnt]))
                    # LAM features used for interspeech table2
                    temp_feat_LAM = np.load(os.path.join(self._lam_feat_dir, self._filenames_list[file_cnt]), mmap_mode='r').transpose(1, 0, 2)

                    n_feats = temp_feat_LAM.shape[0]
                    self._LAM_bands, self._LAM_N_px = temp_feat_LAM.shape[1], temp_feat_LAM.shape[2]
                    temp_feat_LAM = temp_feat_LAM.reshape(n_feats, -1) # (Nsamp, N_band*N_px)
                    #print("is not per file", self._per_file, temp_feat.shape, temp_label.shape, temp_feat_LAM.shape)
                    if not self._per_file: 
                        ...
                        temp_feat_LAM = temp_feat_LAM[:temp_mul*self._label_seq_len, :]
                    
                    #if self._per_file:
                    #    print("Shapes of feats and labels", temp_label.shape, temp_feat_LAM.shape)

                    for f_row in temp_feat:
                        self._circ_buf_feat.append(f_row)
                    for l_row in temp_label:
                        self._circ_buf_label.append(l_row)
                    for f_row in temp_feat_LAM:
                        self._circ_buf_feat_LAM.append(f_row)
                    #TODO: left here before going to work
                ...
                #if self._per_file:
                #    print("happened with labels")
                # Split to sequences
                #feat = self._split_in_seqs(feat, self._feature_seq_len)
                #feat = np.transpose(feat, (0, 2, 1, 3))

                # Split LAM features to sequences
                feat_LAM = self._split_in_seqs(feat_LAM, self._label_seq_len) # (128, 50, 9, 484) 
