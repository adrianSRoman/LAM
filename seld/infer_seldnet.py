...
def get_multi_accdoa_labels(accdoa_in, nb_classes):
    ...
    sed0 = np.sqrt(x0**2 + y0**2 + z0**2) > 0.1
    ...
    sed1 = np.sqrt(x1**2 + y1**2 + z1**2) > 0.1
    ...
    sed2 = np.sqrt(x2**2 + y2**2 + z2**2) > 0.1
...
def test_epoch(data_generator, model, criterion, dcase_output_folder, params, device):
    ...
    file_cnt = 0
    with torch.no_grad():
        for data, target, lam_doa in data_generator.generate():
            # load one batch of data
            data, target, lam_doa = torch.tensor(data).to(device).float(), torch.tensor(target).to(device).float(), torch.tensor(lam_doa).to(device).float()
            
            # process the batch of data based on chosen mode
            output = model(data, lam_doa)
            ...
            if params['multi_accdoa'] is True:
                for frame_cnt in range(sed_pred0.shape[0]):
                    for class_cnt in range(sed_pred0.shape[1]):
                        ...
                        if flag_0sim1 + flag_1sim2 + flag_2sim0 == 0:
                            if sed_pred0[frame_cnt][class_cnt]>0.1:
                            ...
                            if sed_pred1[frame_cnt][class_cnt]>0.1:
                                if frame_cnt not in output_dict:
                                    output_dict[frame_cnt] = []
                            ...
                            if sed_pred2[frame_cnt][class_cnt]>0.1:
                                if frame_cnt not in output_dict:
                                    output_dict[frame_cnt] = []
                ...
                for frame_cnt in range(sed_pred.shape[0]):
                    for class_cnt in range(sed_pred.shape[1]):
                        if sed_pred[frame_cnt][class_cnt]>0.1:
                            if frame_cnt not in output_dict:
                                output_dict[frame_cnt] = []
...
def main(argv):
    ...
    if params['mode'] == 'dev':
        ...
        elif '20.1' in params['dataset_dir']:
            test_splits = [[4]]
            val_splits = [[4]]
            train_splits = [[1, 2, 3]] 
        ...
        elif 'STARSS23' in params['dataset_dir']:
            print("entering here??")
            test_splits = [[5]]
            val_splits = [[4]]#[[4]]
            train_splits = [[1, 2, 3]]

        elif 'spatial_seld_lam_data' in params['dataset_dir']:
            test_splits = [[0]]
            val_splits = [[6]]#[[4]]
            train_splits = [[1, 2, 3, 4]]
        ...
        if 'STARSS23' in params['dataset_dir']:
            print("entering here??")
            test_splits = [[4]]
            val_splits = [[4]]#[[4]]
            train_splits = [[1, 2, 3]]
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        unique_name = '{}_{}_{}_split{}_{}_{}'.format(
            task_id, job_id, "dev", split_cnt, loc_output, loc_feat
        )
        model_name = '{}_model_gru_only_starss4ch_best.h5'.format(os.path.join(params['model_dir'], unique_name))
        print("unique_name: {}\n".format(unique_name))
        ...
        optimizer = AdamW(model.parameters(), lr=params['lr'], weight_decay=0.01)
        if params['multi_accdoa'] is True:
...
#            # Print stats
#            print(
#                'epoch: {}, time: {:0.1f}/{:0.1f}, '
#                # 'train_loss: {:0.1f}, val_loss: {:0.1f}, '
#                'train_loss: {:0.1f}, val_loss: {:0.1f}, '
#                'ER/F/LE/LR/SELD: {}, '
#                'best_val_epoch: {} {}'.format(
#                    epoch_cnt, train_time, val_time,
#                    train_loss, val_loss,
#                    '{:0.1f}/{:0.1f}/{:0.1f}/{:0.1f}/{:0.1f}'.format(val_ER, val_F, val_LE, val_LR, val_seld_scr),
#                    best_val_epoch, '({:0.1f}/{:0.1f}/{:0.1f}/{:0.1f}/{:0.1f})'.format(best_ER, best_F, best_LE, best_LR, best_seld_scr))
#            )
#
#            patience_cnt += 1
...
#                break
        ...
        print('\nTest Loss')
        print('SELD score (early stopping metric): {:0.1f} {}'.format(test_seld_scr[0] if use_jackknife else test_seld_scr, '[{:0.1f}, {:0.1f}]'.format(test_seld_scr[1][0], test_seld_scr[1][1]) if use_jackknife else ''))
        print('SED metrics: Error rate: {:0.1f} {}, F-score: {:0.1f} {}'.format(test_ER[0]  if use_jackknife else test_ER, '[{:0.1f}, {:0.1f}]'.format(test_ER[1][0], test_ER[1][1]) if use_jackknife else '', 100* test_F[0]  if use_jackknife else 100* test_F, '[{:0.1f}, {:0.1f}]'.format(100* test_F[1][0], 100* test_F[1][1]) if use_jackknife else ''))
        print('DOA metrics: Localization error: {:0.1f} {}, Localization Recall: {:0.1f} {}'.format(test_LE[0] if use_jackknife else test_LE, '[{:0.1f} , {:0.1f}]'.format(test_LE[1][0], test_LE[1][1]) if use_jackknife else '', 100*test_LR[0]  if use_jackknife else 100*test_LR,'[{:0.1f}, {:0.1f}]'.format(100*test_LR[1][0], 100*test_LR[1][1]) if use_jackknife else ''))
        if params['average']=='macro':
            print('Classwise results on unseen test data')
            print('Class\tER\tF\tLE\tLR\tSELD_score')
            for cls_cnt in range(params['unique_classes']):
                print('{}\t{:0.1f} {}\t{:0.1f} {}\t{:0.1f} {}\t{:0.1f} {}\t{:0.1f} {}'.format(
                     cls_cnt,
                     classwise_test_scr[0][0][cls_cnt] if use_jackknife else classwise_test_scr[0][cls_cnt], '[{:0.1f}, {:0.1f}]'.format(classwise_test_scr[1][0][cls_cnt][0], classwise_test_scr[1][0][cls_cnt][1]) if use_jackknife else '',
                     classwise_test_scr[0][1][cls_cnt] if use_jackknife else classwise_test_scr[1][cls_cnt], '[{:0.1f}, {:0.1f}]'.format(classwise_test_scr[1][1][cls_cnt][0], classwise_test_scr[1][1][cls_cnt][1]) if use_jackknife else '',
                     classwise_test_scr[0][2][cls_cnt] if use_jackknife else classwise_test_scr[2][cls_cnt], '[{:0.1f}, {:0.1f}]'.format(classwise_test_scr[1][2][cls_cnt][0], classwise_test_scr[1][2][cls_cnt][1]) if use_jackknife else '',
                     classwise_test_scr[0][3][cls_cnt] if use_jackknife else classwise_test_scr[3][cls_cnt], '[{:0.1f}, {:0.1f}]'.format(classwise_test_scr[1][3][cls_cnt][0], classwise_test_scr[1][3][cls_cnt][1]) if use_jackknife else '',
                     classwise_test_scr[0][4][cls_cnt] if use_jackknife else classwise_test_scr[4][cls_cnt], '[{:0.1f}, {:0.1f}]'.format(classwise_test_scr[1][4][cls_cnt][0], classwise_test_scr[1][4][cls_cnt][1]) if use_jackknife else ''))
