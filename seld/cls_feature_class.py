...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        self._hop_len = np.int64(self._fs * self._hop_len_s)

        self._label_hop_len_s = params['label_hop_len_s']
        self._label_hop_len = np.int64(self._fs * self._label_hop_len_s)
        self._label_frame_res = self._fs / float(self._label_hop_len)
        self._nb_label_frames_1s = np.int64(self._label_frame_res)

        self._win_len = int(2 * self._hop_len)
        ...
        if self._use_salsalite and self._dataset=='mic':
            ...
            self._lower_bin = np.int64(np.floor(params['fmin_doa_salsalite'] * self._nfft / np.float64(self._fs)))
            self._lower_bin = np.max((1, self._lower_bin))
            self._upper_bin = np.int64(np.floor(np.min((params['fmax_doa_salsalite'], self._fs//2)) * self._nfft / np.float64(self._fs)))
            ...
            self._cutoff_bin = np.int64(np.floor(params['fmax_spectra_salsalite'] * self._nfft / np.float64(self._fs)))
        ...
        model_checkpoint_path = f"/scratch/data/repos/LAM/checkpoints/{params['pretrained_lam_weights']}"
        device = 'cuda:0'
        self.model = initialize_config(model_config['model'])
        self.model.load_state_dict(load_checkpoint(model_checkpoint_path, device))
    ...
    def get_frame_stats(self):
        ...
        for sub_folder in os.listdir(self._aud_dir):
            ...
            for file_cnt, file_name in enumerate(os.listdir(loc_aud_folder)):
                ...
                nb_feat_frames = np.int64(audio_len / float(self._hop_len))
                nb_label_frames = np.int64(audio_len / float(self._label_hop_len))
                self._filewise_frames[file_name.split('.')[0]] = [nb_feat_frames, nb_label_frames]
        return

    def _load_audio(self, audio_path):
        fs, audio = wav.read(audio_path)
        if audio.shape[1] == 32: # pick 4 channels corresponding to tetra mic
            audio = audio[:, [5,9,25,21]] / 32768.0 + self._eps
        else:
    ...
    def extract_visibilities(self, _data, _rate, T, fc, bw, alpha):
        """
        ...
        """
        N_stft_sample = np.int64(_rate * T)
        if N_stft_sample == 0:
        ...
        idx_start = np.int64((fc - 0.5 * bw) * N_stft_sample / _rate)
        idx_end = np.int64((fc + 0.5 * bw) * N_stft_sample / _rate)
    ...
    def form_visibility(self, data, rate, fc, bw, T_sti, T_stationarity):
        '''
        Parameter
        ---------
        ...
        '''
        S_sti = (self.extract_visibilities(data, rate, T_sti, fc, bw, alpha=1.0))

        N_sample, N_channel = data.shape
        N_sti_per_stationary_block = np.int64(T_stationarity / T_sti)
    ...
    def _get_LAM_for_file(self, audio_filename):
        ...
        for i in range(nbands-1):
            ...
            S = self.form_visibility(audio_in, fs, freq[i], bw, T_sti, T_stationarity)
            N_sample = S.shape[0]
        ...
        visibilities = np.transpose(visibilities, (1, 0, 2, 3))
        batch_size = 128
        chunks = np.array_split(visibilities, np.arange(batch_size, len(visibilities), batch_size))
        LAM_chunks = []
        with torch.no_grad():
            for i, chunk in enumerate(chunks):
                S_in = chunk
                S_in = torch.from_numpy(S_in).to('cuda:0')
                S_out, I_pred = self.model(S_in)
                LAM_chunks.append(I_pred.cpu().detach().numpy())
        LAM_maps = np.concatenate(LAM_chunks, axis=0)
        LAM_maps = np.transpose(LAM_maps, (1, 0, 2))
        return LAM_maps 
    ...
    def _get_spectrogram_for_file(self, audio_filename):
        ...
        nb_feat_frames = np.int64(len(audio_in) / float(self._hop_len))
        nb_label_frames = np.int64(len(audio_in) / float(self._label_hop_len))
    ...
    def extract_file_feature_visibility_matrices(self, _arg_in):
                _file_cnt, _wav_path, _feat_path = _arg_in

                # Extract visibility matrix (DeepWave's input)
                LAM_feats = self._get_LAM_for_file(_wav_path)
                print(_wav_path, LAM_feats.shape)
                if LAM_feats is not None:
                    # print('Compute visibility matrix info {}: {}, {}'.format(_file_cnt, os.path.basename(_wav_path), visibility_mat.shape ))
                    np.save(_feat_path, LAM_feats)
...
#            pool.join()
    ...
    def extract_features_LAM(self):
        # setting up folders
        self._feat_dir = self.get_lam_feat_dir()
        create_folder(self._feat_dir)
        from multiprocessing import Pool
        import time
        start_s = time.time()
        ...
        for sub_folder in os.listdir(self._aud_dir):
            loc_aud_folder = os.path.join(self._aud_dir, sub_folder)
            for file_cnt, file_name in enumerate(os.listdir(loc_aud_folder)):
                if file_name == ".wav" or "._" in file_name:
                    continue
                wav_filename = '{}.wav'.format(file_name.split('.')[0])
                wav_path = os.path.join(loc_aud_folder, wav_filename)
                lam_feat_path = os.path.join(self._feat_dir, '{}.npy'.format(wav_filename.split('.')[0]))
                if os.path.exists(lam_feat_path) and os.path.getsize(dw_feat_path) > 0: # skip if the file is already processed
                    continue

                self.extract_file_feature_visibility_matrices((file_cnt, wav_path, lam_feat_path))
    ...
    def load_output_format_file(self, _output_format_file):
        ...
        for _line in _fid:
            ...
            if len(_words) == 5:  # frame, class idx, source_id, polar coordinates(2) # no distance data, for example in synthetic data fold 1 and 2
                _output_dict[_frame_ind].append([int(_words[1]), np.int64(_words[2]), float(_words[3]), float(_words[4])])
            if len(_words) == 6: # frame, class idx, source_id, polar coordinates(2), distance 
                ...
                _output_dict[_frame_ind].append([int(_words[1]), np.int64(_words[2]), float(_words[3]), float(_words[4]), float(_words[5])])
        _fid.close()
        return _output_dict
    ...
    def write_output_format_file(self, _output_format_file, _output_format_dict):
        ...
        for _frame_ind in _output_format_dict.keys():
            for _value in _output_format_dict[_frame_ind]:
                ...
                _fid.write('{},{},{},{},{},{},{}\n'.format(int(_frame_ind), np.int64(_value[0]), 0, float(_value[1]), float(_value[2]), float(_value[3]), 0))
        _fid.close()
    ...
    def segment_labels(self, _pred_dict, _max_frames):
        ...
        '''
        nb_blocks = np.int64(np.ceil(_max_frames/float(self._nb_label_frames_1s)))
    ...
    def get_lam_feat_dir(self):                                    
        return os.path.join(                                      
            self._feat_label_dir,                                   
            '{}'.format('{}_lam'.format(self._dataset_combination))
        )
